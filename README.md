...
**One token, one machine.** Do not run the same bot token on multiple devices simultaneously. Telegram delivers each update to a single `getUpdates` caller — whichever instance polls last consumes the update, so messages split unpredictably between machines and sessions break mid-conversation. If you want the bridge on more than one host, create a separate bot per machine via BotFather.
